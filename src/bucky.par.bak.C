...
int main(int argc, char *argv[])
{
  ...
  //if ((my_rank == 0) && ((rp.getNumRuns() * rp.getNumChains()) < p)){
  //    cout << rp.getNumRuns() <<" * "<< rp.getNumChains() << " = " << rp.getNumRuns() * rp.getNumChains() << "\n"; 
  //    cerr << "Warning: Total number of chains is less than number of parallel processes!\n";
  //}
  //rp.MPI_calcMyChains(p, my_rank); 
  //cout << flush; 
  //cout << "Rank " << my_rank << " has " << rp.getNumRuns() << " runs and " << rp.getNumChains() << " chains!\n";
  ...
  MPI_Scatter(global_seeds.data(), 2, MPI_INT, local_seeds, 2, MPI_INT, 0, MPI_COMM_WORLD);
...
//  fout << "done." << endl;
  ...
  //TKC:Currently working on altering for MPI version
  
  
  /*PLAN:
   Have rank 0 make array of chain indices and run indices 
   Scatter to each process. Each process then builds a "Start" and "end" 
    for both chains and runs 
   Then alter the below to go from i=start to i=end
    e.g. rank 1 could have local_chains=[2,3] and local_runs=[1,1]
    so, alphas will be assigned as 2*multiplier and 3*multiplier*/
  
  //Build series of vectors holding info for all runs
  //if (my_rank==0){
  int total = rp.getNumChains()*rp.getNumRuns(); 
  int start = my_rank*(total/p); 
  int end = (total/p)+my_rank*(total/p); 
  if (my_rank == p-1)
      end += total%p; 
  cout << "Rank "<<my_rank<<" starts at "<<start<<" and ends at "<<end<<endl;
    ...
    vector<vector<State*> > local_states; 
    //Make serial list of indices for scattering
    vector<int> global_index(rp.getNumRuns());
    vector<int> global_runs(rp.getNumRuns());
    ...
    for (int irun=0; irun<rp.getNumRuns(); irun++){
        ...
        global_index.resize(rp.getNumChains());
        global_runs.resize(rp.getNumChains());
        for(int i=0;i<rp.getNumChains();i++){
            
            //
            if (i==0){
                global_alphas[i+(irun*rp.getNumChains())] = mp.getAlpha();
            }else{
            ...
            //Store chain indices
            global_index[irun+i] = i;
            global_runs[irun+i] = irun; 
        }
    }
        cout << "done." << endl;
        fout << "done." << endl;
  ...
  //int my_chains=();
  //MPI_Scatter(global_alphas.data(), 2, MPI_INT, local_seeds, 2, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  //TKC: Here, will only need to edit 1D vector. 
